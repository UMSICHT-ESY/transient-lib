...
//________________________________________________________________________________//
  ...
  parameter SI.SpecificHeatCapacity cp_storage=4180 "Heat capacity of storage medium" annotation (HideResult=true, Dialog(group="Storage"));
  ...
  Modelica.Blocks.Math.Add addGeneration annotation (Placement(transformation(extent={{26,14},{42,30}})));

  TransiEnt.Producer.Combined.SmallScaleCHP.SmallScaleCHP_simple.SmallScaleCHP_simple smallScaleCHP_simple(
    useFluidPorts=false,
    useHeatPort=false,
    usePowerPort=useElectricityPort,
    change_sign=true,
  ...
  replaceable TransiEnt.Producer.Combined.SmallScaleCHP.SmallScaleCHP_simple.Control.ControlBoilerCHP_modulatingBoiler_HeatLed controller(control_SoC=true)
                                                                                                                                          annotation (
...
equation
  ...
  connect(smallScaleCHP_simple.Q_flow_gen, addGeneration.u2) annotation (Line(
      points={{18.8,-6.6},{18.8,-6},{22,-6},{22,17.2},{24.4,17.2}},
      color={175,0,0},
  ...
  connect(gasBoilerGasAdaptive.Q_flow_gen, addGeneration.u1) annotation (Line(
      points={{-15,-39.2},{-10,-39.2},{-10,26.8},{24.4,26.8}},
      color={175,0,0},
      pattern=LinePattern.Dash));
  ...
  connect(controller.Q_flow_set_CHP, smallScaleCHP_simple.Q_flow_set) annotation (Line(
      ...
      color={175,0,0},
      pattern=LinePattern.Dash));
  connect(addGeneration.y, Storage.Q_flow_store) annotation (Line(points={{42.8,22},{52,22},{52,32},{58.6,32}},
                                                                                                          color={0,0,127}));
  connect(heatDemand, Storage.Q_flow_demand) annotation (Line(
      points={{0,104},{0,60},{84,60},{84,32},{78,32}},
      color={175,0,0},
      pattern=LinePattern.Dash));
  connect(Storage.SoC, controller.SoC) annotation (Line(points={{69.8,41.6},{68,41.6},{68,46},{-70,46},{-70,6},{-65.8,6}}, color={0,0,127}));
...
end CHP_Boiler_Storage_DistrictHeating;
