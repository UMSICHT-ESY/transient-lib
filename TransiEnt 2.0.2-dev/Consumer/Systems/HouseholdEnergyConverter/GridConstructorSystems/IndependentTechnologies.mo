within TransiEnt.Consumer.Systems.HouseholdEnergyConverter.GridConstructorSystems;
...
//________________________________________________________________________________//
  ...
  extends TransiEnt.Consumer.Systems.HouseholdEnergyConverter.GridConstructorSystems.Base.PartialTechnologies(onlyElectric=false);
  ...
  //Boiler parameters
  parameter Modelica.Units.SI.Efficiency eta_boiler=0.9 "Efficiency of the gas, oil or biomass boiler" annotation (HideResult=true);

  //PV parameters
  parameter Modelica.Units.SI.Power P_inst_PV=200 "Combined installed PV power" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.Energy E_max_PV=0 "Maximum capacity of the battery" annotation (HideResult=true);
  parameter Modelica.Units.SI.Energy E_min_PV=0 "Maximum capacity of the battery" annotation (HideResult=true);
  parameter Modelica.Units.SI.Power P_load_PV=2000 "Charging/discharging power of the battery" annotation (HideResult=true);
  parameter Real eta_load_battery_PV=0.95 "Conversion efficiency while loading" annotation (HideResult=true);
  parameter Modelica.Units.SI.Frequency selfDischargeRate_battery=4e-9 "E.g. 0.5/3600 = 50% discharge per hour, used if no detailed staionary loss model is available" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.Power Q_CHP=4000 "Heat output of CHP" annotation (HideResult=true);
  parameter Modelica.Units.SI.Power P_CHP=8000 "Electric power output of CHP" annotation (HideResult=true);
  parameter Modelica.Units.SI.Efficiency eta_boiler_CHP=1.05 "Boiler's overall efficiency" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_s_max_CHP=363.15 "Maximum storage temperature" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_s_min_CHP=303.15 "Minimum storage temperature" annotation (HideResult=true);
  parameter Modelica.Units.SI.Volume V_s_CHP=0.5 "Volume of the Storage" annotation (HideResult=true);
  parameter Modelica.Units.SI.Height h_s_CHP=1.3 "Height of heat storage" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.SurfaceCoefficientOfHeatTransfer k_s_CHP=0.08 "Coefficient of heat Transfer" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.Temperature T_set_ST=348.15 "Temperature set point for controller" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_max_ST=273.15 + 95 "maximum input temperature for collector switch-off" annotation (HideResult=true);
  parameter Modelica.Units.SI.Area area_ST=5 "Aperture area" annotation (HideResult=true);
  parameter Modelica.Units.SI.Volume V_ST=2 "Volume of the storage tank" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_return_ST=308.15 "Return temperature of the heating system" annotation (HideResult=true);
  parameter Real eta_Boiler_ST=0.9 "efficiency of the boiler for the solar thermal system" annotation (HideResult=true);

  parameter Modelica.Units.SI.Angle slope_ST=53.55 "slope of the tilted surface, assumption" annotation (HideResult=true);
  parameter Modelica.Units.SI.Angle azimuth_ST=0 "surface azimuth angle" annotation (HideResult=true);

  parameter Modelica.Units.SI.Angle latitude_ST=53.55 "latitude of the local position, north posiive, 53,55 North for Hamburg" annotation (HideResult=true);
  parameter Modelica.Units.SI.Angle longitude_standard_ST=15 "needed for calculation of coordinated universal time (utc), 15 for central european time, 30 for central european summer time" annotation (HideResult=true);
  parameter Modelica.Units.SI.Angle longitude_local_ST=10 "longitude of the local position, east positive, 10 East for Hamburg" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_set_boiler_ST=60 + 273.15 "Temperature setpoint of the boiler" annotation (HideResult=true);
  // parameter IntegraNet.Basics.Types.FuelType fuel_ST=IntegraNet.Basics.Types.FuelType.Gas "choice of fuel";

  //Heat pump parameters
  parameter Modelica.Units.SI.HeatFlowRate Q_flow_n_HP=3.5e3 "Nominal heat flow of heat pump at nominal conditions according to EN14511" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.Temperature T_s_min_HP=313.15 "Minimum storage temperature of heat pump system" annotation (HideResult=true);
  parameter Modelica.Units.SI.Temperature T_s_max_HP=303.15 "Minimum storage temperature of heat pump system" annotation (HideResult=true);
  parameter Modelica.Units.SI.Volume V_s_HP=0.2 "Volume of the storage of heat pump system" annotation (HideResult=true);
  parameter Modelica.Units.SI.Height h_s_HP=0.5 "Height of heat storage in heat pump system" annotation (HideResult=true);
  ...
  parameter Modelica.Units.SI.SurfaceCoefficientOfHeatTransfer k_s_HP=0.08 "Coefficient of heat transfer through tank surface in heat pump system" annotation (HideResult=true);
  parameter String T_source_type_HP "Temperature of heat source" annotation (HideResult=true);
  Modelica.Units.SI.Temperature T_source_HP "Temperature of heat source" annotation (HideResult=true);
  Modelica.Units.SI.Temperature T_source_ground=simCenter.T_ground + 273.15 "Temperature of ground as heat source" annotation (Dialog(group="Heat pump"));
  Modelica.Units.SI.Temperature T_source_ambient=simCenter.ambientConditions.temperature.value + 273.15 "Temperature of ambient air as heat source" annotation (Dialog(group="Heat pump"));
  Modelica.Units.SI.Temperature T_source_constant=283.15 "Constant heat source temperature" annotation (Dialog(group="Heat pump"));
  Modelica.Units.SI.Temperature T_source_other=283.15 "Other heat source temperature" annotation (Dialog(group="Heat pump"));
  parameter Modelica.Units.SI.Power P_el_backup_HP=10e3 "Nominal electric power of the backup heater" annotation (HideResult=true);
  ...
  TransiEnt.Components.Boundaries.Electrical.ApparentPower.ApparentPower Electric_Consumer(
...
public
 ...
 //Statistics
  ...
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.CollectElectricPower collectElectricPower(typeOfResource=TransiEnt.Basics.Types.TypeOfResource.Consumer) annotation (Placement(transformation(extent={{80,80},{100,100}})));
