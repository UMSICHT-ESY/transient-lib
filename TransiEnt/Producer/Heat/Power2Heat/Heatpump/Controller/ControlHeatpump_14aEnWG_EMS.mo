...
model ControlHeatpump_14aEnWG_EMS "Operation preferably when excess PV energy available, if bivalent mode selected, heater will switch on additionally to heatpump"
...
//________________________________________________________________________________//
  ...
  Modelica.Blocks.Math.Product product annotation (Placement(transformation(extent={{-330,50},{-310,70}})));
  ...
  Modelica.Blocks.Math.Max max annotation (Placement(transformation(extent={{-380,84},{-360,104}})));
  Modelica.Blocks.Sources.RealExpression Mindestleistung(y=7560) if
                                                               not Modulating annotation (Placement(transformation(extent={{-440,72},{-424,88}})));
  Modelica.Blocks.Interfaces.RealInput COP annotation (Placement(transformation(extent={{-470,34},{-430,74}})));
  Modelica.Blocks.Interfaces.RealInput P_SVE annotation (Placement(transformation(extent={{-470,80},{-430,120}})));
  ...
  Modelica.Blocks.Sources.RealExpression uHigh5(y=100)           annotation (Placement(transformation(extent={{-8,-7},{8,7}},
        rotation=90,
        origin={20,-15})));
  Modelica.Blocks.Math.Add3 limiter_EV(k2=-1, k3=-1) annotation (Placement(transformation(extent={{-280,-130},{-260,-110}})));
  Modelica.Blocks.Math.Add Addition annotation (Placement(transformation(extent={{-412,64},{-392,84}})));
  Modelica.Blocks.Interfaces.RealInput PV_Speicher annotation (Placement(transformation(extent={{-468,-120},{-428,-80}})));
  Modelica.Blocks.Math.Gain Multiplikator(k=-1) annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-420,-50})));
  Modelica.Blocks.Math.Gain Multiplikator1(k=2*0.8)
...
equation
  ...
  connect(division.u2,COP)  annotation (Line(points={{-338,-42},{-386,-42},{-386,54},{-450,54}},                         color={0,0,127}));
  connect(max.y, product.u1) annotation (Line(points={{-359,94},{-350,94},{-350,66},{-332,66}}, color={0,0,127}));
  connect(feedback.u1, product.u1) annotation (Line(points={{-318,-2},{-350,-2},{-350,66},{-332,66}}, color={0,0,127}));
  connect(product.y, limiter_HP.limit1) annotation (Line(points={{-309,60},{-292,60}}, color={0,0,127}));
  ...
  connect(limiter_HP.u, firstOrder2.u) annotation (Line(points={{-292,52},{-304,52},{-304,40},{-360,40},{-360,20},{-474,20},{-474,162},{-30,162},{-30,14}},
                                                                                                                  color={0,0,127}));
  ...
  connect(Addition.y, max.u2) annotation (Line(points={{-391,74},{-388,74},{-388,88},{-382,88}}, color={0,0,127}));
  connect(Mindestleistung.y, Addition.u1) annotation (Line(points={{-423.2,80},{-414,80}}, color={0,0,127}));
  connect(limiter_EV.y, P_EV) annotation (Line(points={{-259,-120},{106,-120}}, color={0,0,127}));
  connect(limiter_EV.u1, product.u1) annotation (Line(points={{-282,-112},{-350,-112},{-350,66},{-332,66}}, color={0,0,127}));
  connect(limiter_EV.u2, feedback.u2) annotation (Line(points={{-282,-120},{-310,-120},{-310,-10}}, color={0,0,127}));
  connect(limiter_EV.u3, switch2.u1) annotation (Line(points={{-282,-128},{-300,-128},{-300,-60},{-260,-60},{-260,-8},{-120,-8},{-120,-106},{46,-106},{46,-88.4},{58.4,-88.4}}, color={0,0,127}));
  connect(product.u2, COP) annotation (Line(points={{-332,54},{-450,54}}, color={0,0,127}));
  connect(Multiplikator.u, PV_Speicher) annotation (Line(points={{-420,-62},{-420,-100},{-448,-100}}, color={0,0,127}));
  connect(Multiplikator.y, Addition.u2) annotation (Line(points={{-420,-39},{-420,68},{-414,68}}, color={0,0,127}));
  connect(Multiplikator1.y, max.u1) annotation (Line(points={{-399,100},{-382,100}}, color={0,0,127}));
  connect(P_SVE, Multiplikator1.u) annotation (Line(points={{-450,100},{-422,100}}, color={0,0,127}));
  annotation (Diagram(coordinateSystem(extent={{-100,-140},{100,100}}), graphics={
        ...
        Text(
          extent={{-250,-114},{-236,-118}},
          textColor={28,108,200},
          textString="P_EV"),
        Text(
          extent={{-250,62},{-228,52}},
          textColor={28,108,200},
          textString="Q_flow_set_HP"),
        Text(
          extent={{-454,30},{-432,20}},
          textColor={28,108,200},
          textString="Q_flow_Soll")}),                                   Icon(
...
end ControlHeatpump_14aEnWG_EMS;
