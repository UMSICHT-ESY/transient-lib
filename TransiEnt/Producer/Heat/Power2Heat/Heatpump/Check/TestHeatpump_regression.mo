...
//________________________________________________________________________________//
  ...
  TransiEnt.Producer.Heat.Power2Heat.Heatpump.Heatpump_regression
    heatpump_regression(
    use_T_storage=true,
    usePowerPort=true,
...
equation
  connect(temperature1.port, sink1.steam_a) annotation (Line(
      ...
      thickness=0.5));
  connect(Q_flow_set.y, heatpump_regression.Set_value) annotation (Line(points=
          {{-60.15,-41},{-28,-41},{-28,-11.4},{-20.6,-11.4}}, color={0,0,127}));
  connect(const.y, heatpump_regression.T_storage) annotation (Line(points={{-59,
          4},{-28,4},{-28,0},{-20.6,0}}, color={0,0,127}));
  connect(heatpump_regression.inlet, source1.steam_a) annotation (Line(
      points={{0,-9.8},{64,-9.8},{64,-22}},
      color={175,0,0},
      thickness=0.5));
  connect(heatpump_regression.outlet, sink1.steam_a) annotation (Line(
      points={{0.2,-3},{62,-3},{62,11}},
      color={175,0,0},
      thickness=0.5));
  connect(electricGrid1.epp, heatpump_regression.epp) annotation (Line(
