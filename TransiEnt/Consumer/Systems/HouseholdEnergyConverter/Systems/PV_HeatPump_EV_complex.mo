...
model PV_HeatPump_EV_complex "PV + Heatpump with thermal storage"
...
//________________________________________________________________________________//
  ...
  parameter Modelica.Units.SI.Power P_el_n=3e3 "Nominal electric power of the backup heater" annotation (HideResult=true, Dialog(group="Heatpump"));
  ...
  Masterarbeit.EnergyConverterMasterarbeit.SinglePhasePVInverter_complex inverter(
    eta=0.98,
    cosphi=0.95,
    behavior=1,
    P_n=12000,
  ...
  TransiEnt.Components.Boundaries.Electrical.ComplexPower.PQBoundary     pQBoundary(    useInputConnectorQ=false, useInputConnectorP=true) annotation (Placement(transformation(extent={{-70,-40},{-54,-24}})));
  ...
  Masterarbeit.EnergyConverterMasterarbeit.ElectricBoiler_complex electricHeater(
    change_sign=true,
    usePelset=true,
    Q_flow_n=P_el_n*0.95,
    eta=0.95,
    useFluidPorts=false,
    usePowerPort=true) annotation (Placement(transformation(extent={{52,-72},{72,-52}})));
  Modelica.Blocks.Math.Add add3 annotation (Placement(transformation(extent={{64,-46},{78,-32}})));

  Modelica.Blocks.Sources.RealExpression Tset(y=heatPump.T_set)
                                                       annotation (Placement(transformation(extent={{-96,-24},{-78,-6}})));

  replaceable Masterarbeit.EnergyConverterMasterarbeit.MaxSelfConsumption controller1 if battery constrainedby TransiEnt.Consumer.Systems.HouseholdEnergyConverter.Systems.Control_Battery.MaxSelfConsumption "Operation strategy of the battery" annotation (
    Dialog(group="Battery Parameters"),
    choicesAllMatching=true,
  ...
  Modelica.Blocks.Sources.RealExpression excessPV1(y=pQBoundary.epp.P + heatPump.epp.P + electricHeater.epp.P) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-130,78})));
  TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort     epp2 if     el_grid annotation (Placement(transformation(extent={{-68,-108},{-48,-88}})));
  Masterarbeit.EnergyConverterMasterarbeit.Heatpump_regression_Julian heatPump(
    use_T_storage=true,
    usePowerPort=true,
    Q_flow_n=10500,
    useFluidPorts=false,
    useHeatPort=false,
    T_set=328.15,
    redeclare connector PowerPortModel = TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort,
    redeclare model PowerBoundaryModel = TransiEnt.Components.Boundaries.Electrical.ComplexPower.PQBoundary,
    Power(useInputConnectorQ=false)) annotation (Placement(transformation(extent={{10,-22},{30,-2}})));
  Masterarbeit.EnergyConverterMasterarbeit.PVModule pVModule(
    P_inst=6000,
    Pmpp=Pmpp,
    Area=Area,
    Strings=Strings,
  ...
  Modelica.Blocks.Sources.RealExpression wind(y=simCenter.ambientConditions.wind.value) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-34,63})));
  Masterarbeit.EnergyConverterMasterarbeit.PVModule1 pVModule1(
    P_inst=6000,
    Pmpp=Pmpp,
    Area=Area,
    Strings=Strings,
  ...
  TransiEnt.Storage.Electrical.LithiumIonBattery
            ...
            eta_load=1), redeclare model CostModel = TransiEnt.Components.Statistics.ConfigurationData.StorageCostSpecs.LithiumIonBattery)
                     annotation (Placement(transformation(extent={{-133,-17},{-111,5}})));
  Modelica.Blocks.Sources.RealExpression storage_SOC(y=Storage.SOC.y) annotation (Placement(transformation(extent={{-174,40},{-156,58}})));
  TransiEnt.Consumer.Electrical.BatteryElectricVehicle
    ...
    redeclare model DistanceLocationTable = TransiEnt.Basics.Tables.ElectricGrid.Electromobility.DistanceLocationProfiles_family_15min (relativepath="emobility/building01.csv"))
...
equation
  ...
  connect(excessPV.y, controller.PV_excess) annotation (Line(points={{-29,3.3},{-29,-3.6}},                    color={0,0,127}));
  ...
  connect(electricHeater.Q_flow_gen, add3.u2) annotation (Line(
      points={{72.6,-53.8},{72.6,-48},{56,-48},{56,-43.2},{62.6,-43.2}},
      color={175,0,0},
      pattern=LinePattern.Dash));
  connect(controller.P_set_electricHeater, electricHeater.P_el_set) annotation (Line(
      points={{-9.5,-21.7},{-9.5,-24},{8,-24},{8,-64},{52.4,-64},{52.4,-64.4}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Tset.y, controller.T_set) annotation (Line(points={{-77.1,-15},{-77.1,-15.4},{-29.2,-15.4}},         color={0,0,127}));
  connect(p_PV.y, controller1.P_PV) annotation (Line(points={{-112,68},{-112,48},{-117.2,48},{-117.2,41.52}}, color={0,0,127}));
  connect(excessPV1.y, controller1.P_Consumer) annotation (Line(points={{-130,69.2},{-130,48},{-126.8,48},{-126.8,41.36}}, color={0,0,127}));
  connect(pQBoundary.epp, epp2) annotation (Line(
      points={{-70,-32},{-74,-32},{-74,-84},{-58,-84},{-58,-98}},
      color={28,108,200},
      thickness=0.5));
  connect(inverter.epp_AC, epp2) annotation (Line(
      points={{-111,-68},{-111,-82},{-74,-82},{-74,-84},{-58,-84},{-58,-98}},
      color={28,108,200},
      thickness=0.5));
  connect(electricHeater.epp, epp2) annotation (Line(
      points={{62,-72.2},{62,-82},{-58,-82},{-58,-98}},
      color={28,108,200},
      thickness=0.5));
  connect(controller.Q_flow_set_HP, heatPump.Set_value) annotation (Line(
      points={{-9.5,-14.1},{3.25,-14.1},{3.25,-17.4},{9.4,-17.4}},
      color={175,0,0},
      pattern=LinePattern.Dash));
  connect(heatPump.Heat_output, add3.u1) annotation (Line(
      ...
      color={175,0,0},
      pattern=LinePattern.Dash));
  connect(heatPump.epp, epp2) annotation (Line(
      points={{27.6,-22},{28,-22},{28,-82},{-58,-82},{-58,-98}},
      color={28,108,200},
      thickness=0.5));
  ...
  connect(heatPump.T_storage, heatStorage1.T_stor_out) annotation (Line(points={{9.4,-6},{2,-6},{2,22},{60,22},{60,26},{110,26},{110,68},{74.2,68},{74.2,55.6}}, color={0,0,127}));
  connect(Storage.epp, inverter.epp_DC) annotation (Line(
      points={{-111,-6},{-106,-6},{-106,-44},{-111,-44},{-111,-50.18}},
      color={0,135,135},
      thickness=0.5));
  connect(controller1.P_set_battery, Storage.P_set) annotation (Line(points={{-122,26},{-122,4.34}}, color={0,0,127}));
  connect(storage_SOC.y, controller1.SOC) annotation (Line(points={{-155.1,49},{-132.72,49},{-132.72,42.16}}, color={0,0,127}));
  connect(batteryElectricVehicle.epp, epp2) annotation (Line(
      points={{-20.2,-48.2},{-6,-48.2},{-6,-82},{-58,-82},{-58,-98}},
      color={28,108,200},
      thickness=0.5));
  annotation (
    HideResult=true,
